...
class BinarySearchTree:
    ...
    def insert(self, value):
        if not self.value:
            return "You need a tree to add to brah!"
        if value < self.value:
            if not self.left:
                self.left = BinarySearchTree(value)
            else:
                self.left.insert(value) # check value against root child's left side
        else:
            if not self.right:
    ...
    def contains(self, target):
        ...
        if self.value == target: # if value equals target, it must be contained in tree! return True
            return True
